<%@ Page Language="C#" AutoEventWireup="true" CodeFile="BCGRecord_Detail.aspx.cs" Inherits="Vaccination_RecordM_BCGRecord_Detail" MasterPageFile="~/MasterPage/Custom/DecoratedMasterPage.master" %>
...
<section class="Content" ng-app="MyApp" ng-controller="MyController">
  ...
  <form id="MyForm" runat="server"  autocomplete="off">
    <div class="formBtn formBtnleft">
         <asp:Button CssClass="btn" ID="btnSave" Text="修改儲存" runat="server" OnClick="btnSave_Click" />
    ...
    <div class="formTb  formTb6">
      <table>
        <tr>
          ...
          <td><asp:TextBox ID="tbBirthNumber" type="number" min="0" ng-model="VM.tbBirthNumber" ng-change="changeAll()" ClientIDMode="Static" runat="server" /></td>
        </tr>
      </table>
    </div>
    <div class="formTb  formTb6">
    ...
    <div class="formTb  formTb6">
      <table>
        <tr>
          ...
          <th rowspan="3" class="width01">國小一年級學童：</th>
          <th scope="row">有疤不需測驗數：</th>
          <td><asp:TextBox ID="tbKidHasScar1" type="number" min="0" ng-model="VM.tbKidHasScar1" ClientIDMode="Static" runat="server" /></td>
        </tr>
        <tr>
          <th scope="row">陽性數：</th>
          ...
          <th scope="row">未驗針及應測未測人數：</th>
          <td> <asp:TextBox ID="tbKidHasScar2" type="number" min="0" ng-model="VM.tbKidHasScar2" ClientIDMode="Static" runat="server" /></td>
        </tr>
        <tr>
          <th scope="row">接種數：</th>
          ...
          <th scope="row">缺席人數：</th>
          <td> <asp:TextBox ID="tbKidHasScar3" type="number" min="0" ng-model="VM.tbKidHasScar3" ClientIDMode="Static" runat="server" /></td>
        </tr>
      </table>
    </div>
    <div class="formTb  formTb6">
  ...
  <tr>
    ...
    <th>在籍學生數：</th>
    <td>
        <asp:TextBox ID="tbAll4"  type="number" min="0" ng-model="VM.all4" Enabled="false"  ClientIDMode="Static" runat="server" />
    </td>
  </tr>
  <tr>
    <th>陽性數：</th>
  ...
  <tr>
    ...
    <td><asp:TextBox ID="tbAll13" type="number" min="0" ng-model="VM.all3"   Enabled="false"  ClientIDMode="Static" runat="server" /></td>
      ...
      <asp:HiddenField ID="hf"    ClientIDMode="Static" runat="server" />
  </form>
</section>
    <script>
        var tbAry=<%=tbAry%>;
        var tbBirthNumber = <%:BirthNumber%>;
        var tbBaby = <%=Baby%>;
        var tbKid = <%=Kid%>;
    </script>
</asp:Content>